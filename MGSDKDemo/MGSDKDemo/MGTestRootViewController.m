...
- (void)initialViews
{
    ...
    static NSString *kXYPlatformBundle = @"AWSDKResources.bundle";
    NSString* bundlePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:kXYPlatformBundle];
    NSString* imagePath = [bundlePath stringByAppendingPathComponent:@"xy_login_logo"];
    
    UIImageView *logoIv = [[UIImageView alloc]initWithImage:[UIImage imageWithContentsOfFile:imagePath]];
    ...
    NSString* line1Path = [bundlePath stringByAppendingPathComponent:@"xy_xline"];
    UIImageView *line1 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:line1Path]];
    ...
    _actionItems = @[@"注册", @"切换帐号",@"注销",@"创建角色",@"角色登录",@"充值统计",@"内购",@"查询用户是否绑定手机",@"查询用户是否绑身份证",@"热更新开始",@"热更新结束"];
...
- (void)MGplatformLeavedPlatform:(NSNotification*)notification
{
    ...
    switch ([leavedType integerValue]) {
        ...
        case MGPlatformLeavedFromSNSCenter:{
            NSLog(@"用户离开平台－－> MGPlatformLeavedFromUserCenter");
            break;
        }
